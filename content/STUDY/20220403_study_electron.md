...
어찌 보면 굉장히 심심한 내용이기도 하고 단순 정보 저장용이란 느낌으로써봤는데 가이드 문서라 그런지 더 심심하단 생각이 들었다. 그럼에도 보면서 알 수 있던 점은

1. 필요한 것, 핵심적으로 먼저 알아야 할 것이 무엇인지를 알 수 있었고
2. 이 API 및 프레임워크를 활용할 때 할 수 일의 수준, 하지 못하는 영역은 어떤 건지 대충은 감이 왔고
3. 해당 API를 공부할 방법을 알 수 있었다.

자바 스크립트 공부를 하겠다고 마음 먹은 이상, 이걸 활용해서 중소형 프로젝트를 하나 해보던지, 좀더 디테일하게 파고 들어 하나 제대로 웹 - 서버까지 깊이를 더한 녀석을 만들지 생각해봐야 할 것 같다. 노마드 코더에서 운영하는 홈페이지에서 클론코딩 강좌로 웹앱 개발이 있던데 그런걸 이용해서 만들어볼까 싶기도 하다.
